...
extension EditApplyOverallViewController: EditApplyOverallPresentableAction {
    ...
    var textCompanyName: Observable<String> {
        return selfView.companyUnderLineTextFieldView.textField.rx.text.orEmpty.debounce(.milliseconds(200), scheduler: MainScheduler.instance).distinctUntilChanged().asObservable()
    }
